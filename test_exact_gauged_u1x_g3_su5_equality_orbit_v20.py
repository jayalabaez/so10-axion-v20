...
def test_report_executes_without_internal_failures():
    ...
    assert report["overall_state"] == "OPEN_GLOBAL_LEMMA"
...
def test_complete_su3_fixed_slice_is_integrated_without_global_overclaim():
    report = gate.build_report()
    ...
    assert report["scope"]["corrected_signed_Phi_orbit_theorem_open"] is True


def test_global_claims_fail_closed_on_precise_remaining_lemma():
    report = gate.build_report()
    assert report["remaining_global_lemma"]["proved"] is False
    ...
    assert (
        report["scope"]["distant_disconnected_Phi_components_excluded"] is False
    )
    ...
    assert report["scope"]["all_arbitrary_Phi_global_equalities_classified"] is False
    assert report["scope"]["local_one_orbit_can_be_strengthened_globally"] is False
    assert report["scope"]["global_equality_orbit_classification_complete"] is False

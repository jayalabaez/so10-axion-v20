...
OUT_JSON = ROOT / "LIVE_G2_EXACT_126BAR_SELF_QUARTIC_DERIVATIVES_V20.json"
OUT_MD = ROOT / "LIVE_G2_EXACT_126BAR_SELF_QUARTIC_DERIVATIVES_V20.md"
BASE_FAMILY = "126bar_self_quartics"
BASIS_LABELS = ("54", "1050bar", "2772bar", "4125")
CHANNELS = BASIS_LABELS
...
def pair_operator() -> sparse.csr_matrix:
    """Matrix of the 126bar pair Casimir in column-major pair coordinates."""
    result = sparse.csr_matrix((source.DIM**2, source.DIM**2), dtype=complex)
    for generator in source._generators():
        result += sparse.kron(sparse.csr_matrix(generator), sparse.csr_matrix(generator), format="csr")
    return result


def _powers_columns(columns: np.ndarray) -> tuple[np.ndarray, ...]:
    powers = [np.asarray(columns, dtype=complex)]
    for _ in range(3):
        powers.append(pair_operator() @ powers[-1])
    return tuple(powers)


def _project_columns(channel: str, columns: np.ndarray) -> np.ndarray:
    powers = _powers_columns(columns)
    return sum(float(coefficient) * powers[index]
               for index, coefficient in enumerate(source._poly(channel)))


def sigma_quartic_derivatives(
    vector: np.ndarray, channel: str
) -> tuple[float, np.ndarray, np.ndarray]:
    """Return ||Pi_q(v v^T)||² and unrestricted canonical-real derivatives."""
    if channel not in CHANNELS:
        raise KeyError(f"unknown 126bar self-quartic channel {channel}")
    v = np.asarray(vector, dtype=complex).reshape(source.DIM)
    pair = np.outer(v, v)
    projected = _project_columns(channel, pair.reshape(-1, 1, order="F")).reshape(
        source.DIM, source.DIM, order="F"
    )
    value = float(np.vdot(projected, projected).real)

    variations = np.empty((source.DIM, source.DIM, chart.SIGMA_REAL_DIM), dtype=complex)
    directions = np.zeros((source.DIM, chart.SIGMA_REAL_DIM), dtype=complex)
    directions[np.arange(source.DIM), 2 * np.arange(source.DIM)] = 1.0 / np.sqrt(2.0)
    directions[np.arange(source.DIM), 2 * np.arange(source.DIM) + 1] = 1j / np.sqrt(2.0)
    variations[:] = (
        np.einsum("i,ja->ija", v, directions, optimize=True)
        + np.einsum("ia,j->ija", directions, v, optimize=True)
    )
    jacobian = _project_columns(
        channel, variations.reshape(source.DIM**2, chart.SIGMA_REAL_DIM, order="F")
    )
    gradient = 2.0 * np.real(np.conjugate(projected.reshape(-1, order="F")) @ jacobian)
    first_term = 2.0 * np.real(jacobian.conj().T @ jacobian)
    # source._second_term is the chart-scaled 2 Re<P,d²P> term: its raw
    # interleaved convention lacks the chart's 1/sqrt(2) per tangent.
    hessian = first_term + source._second_term(projected)
...
def base_derivative(q: np.ndarray, basis_index: int) -> tuple[complex, np.ndarray, np.ndarray]:
    ...
    if index not in range(len(BASIS_LABELS)):
        raise KeyError(f"unknown 126bar self-quartic basis index {basis_index}")
    coordinates = np.asarray(q, dtype=float).reshape(chart.TOTAL_DIM)
    sigma = chart._unpack_complex_interleaved(coordinates[chart.SIGMA_SLICE])
    value, local_gradient, local_hessian = sigma_quartic_derivatives(sigma, BASIS_LABELS[index])
    gradient = np.zeros(chart.TOTAL_DIM, dtype=complex)
    hessian = np.zeros((chart.TOTAL_DIM, chart.TOTAL_DIM), dtype=complex)
    gradient[chart.SIGMA_SLICE] = local_gradient
    hessian[chart.SIGMA_SLICE, chart.SIGMA_SLICE] = local_hessian
...
def build_report() -> dict[str, Any]:
    state = potential.deterministic_state(3504)
    q = chart.pack(state)
    ...
    directional = quadratic.five_point_directional_audit(state, parameters, quadratic.deterministic_coefficients(parameters))
    ...
    self_imaginary = max([max(abs(row.value.imag), float(np.max(np.abs(row.gradient.imag))),
                              float(np.max(np.abs(row.hessian.imag)))) for row in analytic if row.self_conjugate] or [0.0])
    basis_indices = sorted({row.basis_index for row in directions})
    basis_labels = [BASIS_LABELS[index] for index in basis_indices]
    ...
    checks = {
        ...
        "all_four_basis_labels_present": basis_labels == list(BASIS_LABELS),
    ...
    return _jsonable({"status": "G2_EXACT_126BAR_SELF_QUARTIC_DERIVATIVES_CLOSED" if not failures else "G2_126BAR_SELF_QUARTIC_DERIVATIVES_FAILED",
        ...
        "flags": {"authoritative_126bar_self_quartic_adapter_closed": not failures,
        ...
        "verdict": "The 54, 1050bar, 2772bar, and 4125 126bar self quartics have exact dense derivatives; G2 remains PARTIAL."})


def write_report(report: dict[str, Any]) -> None:
    OUT_JSON.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    OUT_MD.write_text("# Exact 126bar self-quartic derivatives\n\n" + report["verdict"] + "\n", encoding="utf-8")

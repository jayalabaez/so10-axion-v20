#!/usr/bin/env python3
"""Exact 486-real derivatives for the unique Hdag Sigma^2 Sigmadag quartic."""
...
OUT_JSON = ROOT / "LIVE_G2_EXACT_HDAG_SIGMA2_SIGMADAG_DERIVATIVES_V20.json"
OUT_MD = ROOT / "LIVE_G2_EXACT_HDAG_SIGMA2_SIGMADAG_DERIVATIVES_V20.md"
BASE_FAMILY = "Hdag_Sigma2_Sigmadag"
BASIS_LABELS = (ledger.BASE_FAMILIES[(0, 0, 1, 2, 1)]["basis"][0],)
...
def build_report() -> dict[str, Any]:
    ...
    return _jsonable({"status": "G2_EXACT_HDAG_SIGMA2_SIGMADAG_DERIVATIVES_CLOSED" if not failures else "G2_HDAG_SIGMA2_SIGMADAG_DERIVATIVES_FAILED",
        ...
        "flags": {"authoritative_Hdag_Sigma2_Sigmadag_adapter_closed": not failures,
        ...
        "verdict": "The unique chiral Hdag Sigma^2 Sigmadag graph has exact canonical-real derivatives; G2 remains PARTIAL."})


def write_report(report: dict[str, Any]) -> None:
    OUT_JSON.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    OUT_MD.write_text("# Exact Hdag Sigma2 Sigmadag derivatives\n\n" + report["verdict"] + "\n", encoding="utf-8")

...
Every charge-neutral scalar multidegree of total degree <= 4 is evaluated by
an exact D5 character calculation.

Representations are built without proxy decompositions:

* ``10``: vector weights ``+-e_i``;
* ``210 = Lambda^4(10)``;
* ``126 = Sym^2(16_chiral)-10``;
* ``126bar``: conjugate character.

Repeated bosonic fields use exact symmetric-power characters via Newton
identities. Singlets are extracted by Weyl highest-weight inversion,

    mult_1(chi) = sum_{w in W(D5)} det(w) mult_chi(w rho-rho).

Weights use doubled orthonormal coordinates. This closes the multiplicity
census, not the explicit normalized component-tensor basis.
"""
...
FIELD_LABEL = {
    "P": "210_H", "H": "10_H", "Hb": "10_H^dag",
    "D": "126bar_H", "Db": "126bar_H^dag",
...
def adams(character: Character, degree: int) -> Character:
    return Counter({scale_weight(weight, degree): mult for weight, mult in character.items()})
...
def symmetric_power_character(character: Character, degree: int) -> Character:
    ...
    for n in range(1, degree + 1):
        ...
        for k in range(1, n + 1):
            add_character(numerator, tensor_character(adams(character, k), complete[n - k]))
        output: Character = Counter()
        for weight, coefficient in numerator.items():
            if coefficient % n:
                raise ArithmeticError(f"Newton coefficient {coefficient} not divisible by {n}")
            output[weight] = coefficient // n
        if any(value < 0 for value in output.values()):
            raise ArithmeticError("negative symmetric-power multiplicity")
...
def exterior_character_from_states(states: Iterable[Weight], degree: int) -> Character:
    ...
    for state in states:
        for n in range(degree, 0, -1):
            shifted = Counter({add_weights(weight, state): mult for weight, mult in powers[n - 1].items()})
...
def rep126_character() -> Character:
    ...
    if any(value < 0 for value in output.values()):
        raise ArithmeticError("Sym^2(16)-10 is not a valid 126 character")
    return output


@lru_cache(maxsize=1)
def rep126bar_character() -> Character:
    return Counter({tuple(-value for value in weight): mult for weight, mult in rep126_character().items()})
...
def d5_weyl_offsets() -> Counter[Weight]:
    ...
    for permutation in itertools.permutations(range(5)):
        ...
        for first_four in itertools.product((-1, 1), repeat=4):
            ...
            moved = tuple(signs[index] * rho[permutation[index]] for index in range(5))
...
def singlet_multiplicity(character: Character) -> int:
    value = sum(signed * character.get(offset, 0) for offset, signed in d5_weyl_offsets().items())
    if value < 0:
        raise ArithmeticError(f"negative singlet multiplicity {value}")
    return int(value)


@lru_cache(maxsize=None)
def symmetric_rep_character(rep: str, degree: int) -> Character:
    base = {"P": rep210_character, "H": vector_character, "Hb": vector_character,
            "D": rep126bar_character, "Db": rep126_character}[rep]()
...
def representation_character(counts: tuple[int, ...]) -> Character:
    ...
    factors = [symmetric_rep_character(field, by_field[field]) for field in ("P", "H", "Hb", "D", "Db") if by_field[field]]
    if not factors:
...
def charge_neutral(counts: tuple[int, ...]) -> bool:
    return total_charge(counts) == {"PQ": 0, "X": 0, "Z17": 0}
...
def census() -> tuple[dict[str, Any], ...]:
    ...
    for degree in range(1, 5):
        for counts in all_compositions(degree, len(FIELD_ORDER)):
            ...
            rows.append({
                "counts": dict(zip(FIELD_ORDER, counts)), "count_tuple": list(counts),
                "degree": degree, "monomial": monomial_label(counts),
                "charge": total_charge(counts), "so10_singlet_multiplicity": multiplicity,
                "conjugate_count_tuple": list(conjugate), "conjugate_monomial": monomial_label(conjugate),
                "self_conjugate": counts == conjugate, "conjugacy_orbit_key": list(min(counts, conjugate)),
...
def orbit_summary(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    by_key: dict[tuple[int, ...], list[dict[str, Any]]] = {}
    for row in rows:
        by_key.setdefault(tuple(row["conjugacy_orbit_key"]), []).append(row)
    output = []
    for key, members in sorted(by_key.items(), key=lambda item: (sum(item[0]), item[0])):
        ...
        output.append({
            "orbit_key": list(key), "representative": monomial_label(key), "degree": sum(key),
            "self_conjugate": self_conjugate, "so10_singlet_multiplicity": multiplicity,
...
def find_multiplicity(rows: Iterable[dict[str, Any]], **counts_by_field: int) -> int:
...
def build_report() -> dict[str, Any]:
    characters = {"10": vector_character(), "16": chiral_spinor_character(),
                  "126": rep126_character(), "126bar": rep126bar_character(), "210": rep210_character()}
    rows = census()
    orbits = orbit_summary(rows)
    multiplicity_by_degree = {degree: sum(row["so10_singlet_multiplicity"] for row in rows if row["degree"] == degree) for degree in range(1, 5)}
    orbit_multiplicity_by_degree = {degree: sum(row["so10_singlet_multiplicity"] for row in orbits if row["degree"] == degree) for degree in range(1, 5)}
    ...
    anchors = {
        ...
        "renormalizable_54_quartic_unique": find_multiplicity(rows, H=2, Db=2),
        "dimensionful_cubic_unique": find_multiplicity(rows, P=1, H=1, Db=1),
        "S_dependent_portal_unique": find_multiplicity(rows, P=1, H=1, D=1, S=1),
        "P2_H_126dag_multiplicity": find_multiplicity(rows, P=2, H=1, Db=1),
        "P2_126bar_126_multiplicity": find_multiplicity(rows, P=2, D=1, Db=1),
        "P2_H_Hdag_multiplicity": find_multiplicity(rows, P=2, H=1, Hb=1),
        "H_Hdag_126bar_126_multiplicity": find_multiplicity(rows, H=1, Hb=1, D=1, Db=1),
        "H2_Hdag2_multiplicity": find_multiplicity(rows, H=2, Hb=2),
    }
    counts = {
        ...
        "total_real_potential_parameters": sum(row["real_parameter_count"] for row in orbits),
    }
    raw_checks = {
        ...
        "renormalizable_54_quartic_unique": anchors["renormalizable_54_quartic_unique"] == 1,
        "dimensionful_cubic_unique": anchors["dimensionful_cubic_unique"] == 1,
        "S_dependent_portal_unique": anchors["S_dependent_portal_unique"] == 1,
        "exact_multidegree_count": counts["charge_and_so10_allowed_multidegrees"] == 34,
        "exact_conjugacy_orbit_count": counts["hermitian_conjugacy_orbits"] == 28,
        "exact_total_complex_multiplicity": counts["total_complex_invariant_multiplicity"] == 51,
        "exact_total_orbit_multiplicity": counts["total_potential_orbit_multiplicity"] == 44,
        "exact_total_real_parameter_count": counts["total_real_potential_parameters"] == 51,
        "conjugate_multiplicities_match": all(len({m["so10_singlet_multiplicity"] for m in rows if m["conjugacy_orbit_key"] == orbit["orbit_key"]}) == 1 for orbit in orbits),
        "whole_model_not_overclaimed": True,
    }
    checks = {name: bool(value) for name, value in raw_checks.items()}
    failures = [name for name, passed in checks.items() if not passed]
    return {
        "status": "EXACT_RENORMALIZABLE_G1_SINGLET_MULTIPLICITY_CENSUS_COMPLETE" if not failures else "EXACT_RENORMALIZABLE_G1_CHARACTER_CENSUS_FAILED",
        "overall_state": "BLOCKED", "n_checks": len(checks), "n_failed": len(failures),
        "failures": failures, "checks": checks,
        "method": {"algebra": "D5 = so(10)", "weight_coordinates": "doubled orthonormal e_i basis",
                   "210": "Lambda^4(10)", "126": "Sym^2(16_chiral)-10",
                   "bosonic_repetition": "exact symmetric-power character via Newton identities",
                   "singlet_extraction": "sum_w det(w) mult(w rho-rho)",
                   "maximum_total_degree": 4, "charge_filter": "PQ=0, X=0, Z17=0"},
        "character_dimensions": {name: character_dimension(char) for name, char in characters.items()},
        "anchors": anchors, "counts": counts, "multidegrees": list(rows), "potential_orbits": orbits,
        "new_exact_multiplicity_findings": {
            "210_H^2 10_H 126bar_H^dag": anchors["P2_H_126dag_multiplicity"],
            "210_H^2 126bar_H 126bar_H^dag": anchors["P2_126bar_126_multiplicity"],
            "210_H^2 10_H 10_H^dag": anchors["P2_H_Hdag_multiplicity"],
            "10_H 10_H^dag 126bar_H 126bar_H^dag": anchors["H_Hdag_126bar_126_multiplicity"],
            "10_H^2 10_H^dag^2": anchors["H2_Hdag2_multiplicity"],
        },
        "closure": {"charge_neutral_multidegree_enumeration_closed": not failures,
        ...
        "flags": {"exact_character_census_without_proxy_multiplicities": not failures,
                  "renormalizable_G1_multiplicity_census_closed": not failures,
                  "g1_explicit_tensor_basis_still_open": True,
                  "whole_model_validated": False, "whole_model_excluded": False},
        "verdict": ("The exact D5 character census finds 34 allowed field multidegrees but 44 independent invariant coefficients up to Hermitian conjugacy, corresponding to 51 real potential parameters. Thus the historical 34-entry floor was not a complete invariant count. Explicit component contractions and normalizations remain open before G1/G2 closure."),
    }


def write_report(report: dict[str, Any]) -> None:
    OUT_JSON.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    lines = ["# Exact renormalizable G1 character census — v20", "", f"**Status:** `{report['status']}`", "", report["verdict"], "", "## Counts", ""]
    lines.extend(f"- `{name}`: `{value}`" for name, value in report["counts"].items())
    lines.extend(["", "## Anchor multiplicities", ""])
    lines.extend(f"- `{name}`: `{value}`" for name, value in report["anchors"].items())
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")

...
def test_charge_contract_and_catalogue_registration():
    ...
    assert audit["present_in_current_catalogue"] is True
    assert audit["catalogue_entry"]["status"] == "ALLOWED"
    assert audit["catalogue_entry"]["feeds_triplet_mass"] is True
...
def test_p_delta_background_impact():
    ...
    assert impact["H_Sigmabar_mixed_block_at_p"]["rank"] == 6
    assert impact["H_Phi_mixed_block_at_DeltaR"]["rank"] == 7
...
def test_report_fail_closed():
    ...
    assert report["flag"]["operator_catalogue_currently_incomplete"] is False
    assert report["flag"]["operator_catalogue_corrected"] is True
    ...
    assert report["flag"]["complete_hessian_reaudit_required"] is True

#!/usr/bin/env python3
"""Regression tests for Phi2 Sigma Sigmadag derivatives."""
from __future__ import annotations

import numpy as np
import pytest

import live_g2_exact_phi2_sigma_sigmadag_derivatives_v20 as mod


@pytest.fixture(scope="module")
def state():
    return mod.potential.deterministic_state(3606)
...
def test_values_shapes_and_symmetry(state, analytic):
    ...
    assert len(analytic) == len(expected) > 0
    for row in analytic:
...
def test_unknown_basis_rejected(state):
    with pytest.raises(KeyError):
        mod.base_derivative(mod.chart.pack(state), 99)

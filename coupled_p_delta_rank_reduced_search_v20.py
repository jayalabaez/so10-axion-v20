#!/usr/bin/env python3
"""Rank-reduced optimizer for the complete coupled P+Delta_R Hessian.

The raw stationarity system has one identically zero singlet row at the exact
P+Delta_R background.  This driver removes redundant rows by SVD before the
SLSQP search, while reusing the source-derived 462-real coefficient matrices
from coupled_p_delta_backreaction_scan_v20.
"""
...
OUT_JSON = ROOT / "COUPLED_P_DELTA_RANK_REDUCED_SEARCH_V20.json"
OUT_MD = ROOT / "COUPLED_P_DELTA_RANK_REDUCED_SEARCH_V20.md"
...
def run_search() -> dict[str, Any]:
    ...
    return {
        ...
        "stationarity_matrix": stationarity.tolist(),
        "reduced_stationarity_matrix": reduced.tolist(),
        "variables": {
...
def build_report() -> dict[str, Any]:
    result = run_search()
    checks = {
        "stationarity_rank_reduced": result["stationarity_rank"] < 2,
    ...
    return {
        "status": (
            "COUPLED_P_DELTA_LOCAL_HESSIAN_PASS__OTHER_FIELDS_OPEN"
            if not failures
            else "COUPLED_P_DELTA_RANK_REDUCED_SEARCH_BLOCKED"
        ),
        ...
        "verdict": (
            "The redundant zero stationarity row is removed before optimizing "
...
def write_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Coupled P + Delta_R rank-reduced search — v20",
            "",
            f"**Status:** `{report['status']}`",
            "",
            report["verdict"],
            "",
            f"- stationarity rank: `{result['stationarity_rank']}`",

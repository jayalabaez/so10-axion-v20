#!/usr/bin/env python3
"""Canonical full-Hessian optimizer for the coupled P+Delta_R vacuum.

The fixed-P Sigma gates use angular positivity operators. Canonical real
second derivatives of terms quadratic in the complex 126bar field carry an
overall factor two. This driver applies that conversion to every Sigma-Sigma
block, keeps the analytically differentiated cross blocks unchanged, imposes
full transverse Phi stationarity, and optimizes the 462-real Hessian.
"""
...
OUT_JSON = ROOT / "COUPLED_P_DELTA_CANONICAL_SEARCH_V20.json"
OUT_MD = ROOT / "COUPLED_P_DELTA_CANONICAL_SEARCH_V20.md"
...
def run_search() -> dict[str, Any]:
    source = base.coefficient_matrices()
    coefficients = {
        key: value for key, value in source.items()
        if key not in {"base", "matrices"}
    }
    coefficients["base"] = source["base"].copy()
    coefficients["base"][base.PHI_DIM:, base.PHI_DIM:] *= 2.0
    coefficients["matrices"] = {}
    for name, matrix in source["matrices"].items():
        corrected = matrix.copy()
        corrected[base.PHI_DIM:, base.PHI_DIM:] *= 2.0
        coefficients["matrices"][name] = corrected
...
def build_report() -> dict[str, Any]:
    ...
    return {
        "status": (
            "COUPLED_P_DELTA_CANONICAL_HESSIAN_PASS__OTHER_FIELDS_OPEN"
            if not failures
            else "COUPLED_P_DELTA_CANONICAL_SEARCH_BLOCKED"
        ),
        ...
        "canonical_conversion": {
            "SigmaSigma_scale": 2.0,
            "cross_scale": 1.0,
            "reason": "canonical real Hessian of z^dag O z is 2 O_R",
        },
        "verdict": (
            "All Sigma-Sigma angular operators are converted to canonical real "
            "second derivatives before the complete gauge-quotiented search."
        ),
    }
...
def write_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Coupled P + Delta_R canonical full-Hessian search — v20",
            "",

#!/usr/bin/env python3
"""Regression tests for the exact real-210 self-quartic derivatives."""
...
def state():
    return mod.potential.deterministic_state(3404)
...
def test_authoritative_values_shapes_and_support(state):
    ...
    expected = mod.source.quartic_invariants(state.phi)
    for index, label in enumerate(mod.BASIS_LABELS):
        ...
        assert np.max(np.abs(gradient[mod.chart.PHI_SLICE].imag)) < 1.0e-12

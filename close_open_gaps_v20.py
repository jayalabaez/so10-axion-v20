#!/usr/bin/env python3
"""Aggregate v20 open gaps after RG, channel-FCNC, and NA62 audits."""
...
def fcnc_absence_theorem():
    ...
    channel=_read("CHANNEL_FCNC_RATES_V20_VERDICT.json"); na62=_read("NA62_POINTWISE_LIMIT_V20_VERDICT.json")
    ...
    return {
        "status":"EXACT_QI_THEOREM__CHANNEL_RATES_AND_NA62_LIMIT_APPLIED__MODEL_CLOSURE_OPEN",
        ...
        "flag":{"exact_qI_theorem_proved":True,"actual_finite_model_fcnc_absence_proved":False,"actual_finite_model_fcnc_suppressed":suppressed,"channel_level_amplitudes_implemented":True,"channel_level_branching_ratios_implemented":True,"official_NA62_pointwise_limit_ingested":True,"hierarchical_benchmark_passes_NA62":not nh["pointwise_excluded_90cl"],"generation_dependent_portal_point_excluded_by_NA62":nc["pointwise_excluded_90cl"],"all_portal_parameter_space_excluded":False,"whole_v20_model_excluded":False,"full_correlated_experimental_likelihood_implemented":False,"component_specific_uv_chiral_currents_derived":False,"proved_for_arbitrary_portals":False},
        "reason_not_closed":"The official NA62 pointwise limit excludes one generation-dependent portal point, but the whole model remains open because the UV portal distribution and component-specific chiral currents are not fixed."
    }
...
def build_report():
    ...
    checks={"conditional_region_available":cf["flag"]["conditional_region_Cf"],"unique_cf_not_overclaimed":not cf["flag"]["unconditional_unique_Cf"],"channel_fcnc_rates_implemented":f["flag"]["channel_level_amplitudes_implemented"] and f["flag"]["channel_level_branching_ratios_implemented"],"na62_limit_ingested":f["flag"]["official_NA62_pointwise_limit_ingested"],"excluded_portal_point_recorded":f["flag"]["generation_dependent_portal_point_excluded_by_NA62"],"whole_model_not_overclaimed":not f["flag"]["whole_v20_model_excluded"],"finite_fcnc_not_overclaimed":not f["flag"]["actual_finite_model_fcnc_absence_proved"],"matrix_rg_not_overclaimed":not rg["flag"]["actual_one_loop_matrix_beta_system_solved"],"two_loop_not_overclaimed":not rg["flag"]["two_loop_so10_complete"],"detection_not_claimed":not det["flag"]["real_37GHz_detection"]}
    fail=[n for n,o in checks.items() if not o]
    return {"status":"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE" if not fail else "OPEN_GAP_AUDIT_FAILED","n_checks":len(checks),"n_failed":len(fail),"failures":fail,"conditional_cf_region":cf,"fcnc_analysis":f,"yukawa_rg_analysis":rg,"ghz37_package":det,"gap_status":{"exact_unique_full_Ce_Cp_Cn":False,"conditional_aligned_Cf_region":True,"channel_level_FCNC_rates":True,"official_NA62_pointwise_limit":True,"generation_dependent_portal_point_excluded":True,"all_portal_parameter_space_excluded":False,"finite_model_tree_FCNC_absence_proved":False,"full_correlated_experimental_FCNC_likelihood":False,"full_common_scale_Yukawa_RG_fit":False,"two_loop_SO10_threshold_closure":False,"real_37GHz_detection":False},"verdict":"The official NA62 zero-mass limit excludes the selected generation-dependent portal counterexample, while the hierarchical universal benchmark survives. Unique C_f, full portal-space exclusion, finite-model FCNC closure, precision RG, threshold matching, and experimental detection remain open."}

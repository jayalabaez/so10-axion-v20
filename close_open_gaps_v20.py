#!/usr/bin/env python3
"""Aggregate v20 open gaps after the strict RG re-audit."""
...
def fcnc_absence_theorem():
    ...
    return {"status":"EXACT_QI_THEOREM__FINITE_MODEL_FCNC_PROXY_ONLY","exact_theorem":"If Q_proj=qI exactly, every unitary mass-basis rotation preserves qI and tree-level axion FCNCs vanish.","finite_hierarchical_benchmark":{"departure_from_q_identity":dep,"lepton_off_diagonal_norm":float(le["off_diagonal_norm"]),"quark_off_diagonal_norm":qo,"exactly_scalar_to_1e_14":dep<=1e-14,"numerically_suppressed_to_1e_8":suppressed,"experimental_FCNC_bound_applied":False,"proxy_bounds_applied":True},"generation_dependent_counterexample":{"lepton_off_diagonal_norm":float(bl["off_diagonal_norm"]),"fcnc_possible":bool(bl["fcnc_possible"])},"flag":{"exact_qI_theorem_proved":True,"actual_finite_model_fcnc_absence_proved":False,"actual_finite_model_fcnc_suppressed":suppressed,"experimental_FCNC_bound_applied":False,"proxy_bounds_applied":True,"proved_for_arbitrary_portals":False},"reason_not_closed":"Finite current is not exactly scalar and no channel-level likelihood is implemented."}
...
def build_report():
    ...
    checks={"conditional_region_available":cf["flag"]["conditional_region_Cf"],"unique_cf_not_overclaimed":not cf["flag"]["unconditional_unique_Cf"],"finite_fcnc_not_overclaimed":not f["flag"]["actual_finite_model_fcnc_absence_proved"],"fcnc_likelihood_not_overclaimed":not f["flag"]["experimental_FCNC_bound_applied"],"matrix_rg_not_overclaimed":not rg["flag"]["actual_one_loop_matrix_beta_system_solved"],"two_loop_not_overclaimed":not rg["flag"]["two_loop_so10_complete"],"detection_not_claimed":not det["flag"]["real_37GHz_detection"]}
    fail=[n for n,o in checks.items() if not o]
    return {"status":"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE" if not fail else "OPEN_GAP_AUDIT_FAILED","n_checks":len(checks),"n_failed":len(fail),"failures":fail,"conditional_cf_region":cf,"fcnc_analysis":f,"yukawa_rg_analysis":rg,"ghz37_package":det,"gap_status":{"exact_unique_full_Ce_Cp_Cn":False,"conditional_aligned_Cf_region":True,"finite_model_tree_FCNC_absence_proved":False,"full_common_scale_Yukawa_RG_fit":False,"two_loop_SO10_threshold_closure":False,"real_37GHz_detection":False},"verdict":"The useful new numerical diagnostics survive, but all precision RG, threshold, finite-FCNC, unique-C_f, and experimental blockers remain open."}

#!/usr/bin/env python3
"""Aggregate v20 open gaps after RG, channel-FCNC, NA62, and TWIST audits."""
...
def fcnc_absence_theorem():
    ...
    channel=_read("CHANNEL_FCNC_RATES_V20_VERDICT.json"); na62=_read("NA62_POINTWISE_LIMIT_V20_VERDICT.json"); twist=_read("TWIST_MASSLESS_LIMIT_V20_VERDICT.json")
    ...
    return {
        "status":"EXACT_QI_THEOREM__CHANNEL_RATES_NA62_AND_TWIST_APPLIED__MODEL_CLOSURE_OPEN",
...
def build_report():
    ...
    checks={"conditional_region_available":cf["flag"]["conditional_region_Cf"],"unique_cf_not_overclaimed":not cf["flag"]["unconditional_unique_Cf"],"channel_fcnc_rates_implemented":f["flag"]["channel_level_amplitudes_implemented"] and f["flag"]["channel_level_branching_ratios_implemented"],"na62_limit_ingested":f["flag"]["official_NA62_pointwise_limit_ingested"],"excluded_portal_point_recorded":f["flag"]["generation_dependent_portal_point_excluded_by_NA62"],"twist_limits_ingested":f["flag"]["TWIST_three_published_asymmetry_limits_ingested"],"twist_counterexample_survives":f["flag"]["generation_dependent_portal_point_passes_all_three_TWIST_cases"],"continuous_twist_likelihood_not_overclaimed":not f["flag"]["continuous_TWIST_asymmetry_likelihood_implemented"],"whole_model_not_overclaimed":not f["flag"]["whole_v20_model_excluded"],"finite_fcnc_not_overclaimed":not f["flag"]["actual_finite_model_fcnc_absence_proved"],"matrix_rg_not_overclaimed":not rg["flag"]["actual_one_loop_matrix_beta_system_solved"],"two_loop_not_overclaimed":not rg["flag"]["two_loop_so10_complete"],"detection_not_claimed":not det["flag"]["real_37GHz_detection"]}
    fail=[n for n,o in checks.items() if not o]
    return {"status":"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE" if not fail else "OPEN_GAP_AUDIT_FAILED","n_checks":len(checks),"n_failed":len(fail),"failures":fail,"conditional_cf_region":cf,"fcnc_analysis":f,"yukawa_rg_analysis":rg,"ghz37_package":det,"gap_status":{"exact_unique_full_Ce_Cp_Cn":False,"conditional_aligned_Cf_region":True,"channel_level_FCNC_rates":True,"official_NA62_pointwise_limit":True,"generation_dependent_kaon_portal_point_excluded":True,"TWIST_three_massless_asymmetry_benchmarks":True,"generation_dependent_muon_portal_point_survives_TWIST_benchmarks":True,"continuous_TWIST_asymmetry_likelihood":False,"all_portal_parameter_space_excluded":False,"finite_model_tree_FCNC_absence_proved":False,"full_correlated_experimental_FCNC_likelihood":False,"full_common_scale_Yukawa_RG_fit":False,"two_loop_SO10_threshold_closure":False,"real_37GHz_detection":False},"verdict":"NA62 excludes the selected generation-dependent kaon portal point. Its muon rate survives all three published TWIST massless asymmetry benchmarks. Unique C_f, full portal-space exclusion, finite-model FCNC closure, continuous experimental likelihoods, precision RG, threshold matching, and detection remain open."}

#!/usr/bin/env python3
"""Aggregate v20 open gaps after the strict RG and channel-FCNC audits."""
...
def fcnc_absence_theorem():
    ...
    channel=_read("CHANNEL_FCNC_RATES_V20_VERDICT.json")
    h=channel["hierarchical_benchmark"]; c=channel["generation_dependent_counterexample"]
    return {"status":"EXACT_QI_THEOREM__CHANNEL_RATES_IMPLEMENTED__FULL_LIKELIHOOD_OPEN","exact_theorem":"If Q_proj=qI exactly, every unitary mass-basis rotation preserves qI and tree-level axion FCNCs vanish.","finite_hierarchical_benchmark":{"departure_from_q_identity":dep,"lepton_off_diagonal_norm":float(le["off_diagonal_norm"]),"quark_off_diagonal_norm":qo,"exactly_scalar_to_1e_14":dep<=1e-14,"numerically_suppressed_to_1e_8":suppressed,"BR_mu_to_e_a":float(h["mu_to_e_a"]["branching_ratio"]),"BR_K_to_pi_a":float(h["K_to_pi_a"]["branching_ratio"]),"experimental_FCNC_bound_applied":False,"channel_level_amplitudes_implemented":True,"channel_level_branching_ratios_implemented":True,"pointwise_experimental_likelihoods_implemented":False},"generation_dependent_counterexample":{"lepton_off_diagonal_norm":float(bl["off_diagonal_norm"]),"fcnc_possible":bool(bl["fcnc_possible"]),"BR_mu_to_e_a":float(c["mu_to_e_a"]["branching_ratio"]),"BR_K_to_pi_a":float(c["K_to_pi_a"]["branching_ratio"])},"channel_rate_artifact":{"status":channel["status"],"references":channel["conventions"],"remaining_for_closure":channel["remaining_for_closure"]},"flag":{"exact_qI_theorem_proved":True,"actual_finite_model_fcnc_absence_proved":False,"actual_finite_model_fcnc_suppressed":suppressed,"channel_level_amplitudes_implemented":True,"channel_level_branching_ratios_implemented":True,"experimental_FCNC_bound_applied":False,"pointwise_experimental_likelihoods_implemented":False,"component_specific_uv_chiral_currents_derived":False,"proved_for_arbitrary_portals":False},"reason_not_closed":"Explicit mu->e a and K->pi a rates are implemented, but component-specific UV chiral currents and pointwise experimental likelihoods are still absent."}
...
def build_report():
    ...
    checks={"conditional_region_available":cf["flag"]["conditional_region_Cf"],"unique_cf_not_overclaimed":not cf["flag"]["unconditional_unique_Cf"],"channel_fcnc_rates_implemented":f["flag"]["channel_level_amplitudes_implemented"] and f["flag"]["channel_level_branching_ratios_implemented"],"finite_fcnc_not_overclaimed":not f["flag"]["actual_finite_model_fcnc_absence_proved"],"pointwise_fcnc_likelihood_not_overclaimed":not f["flag"]["pointwise_experimental_likelihoods_implemented"],"matrix_rg_not_overclaimed":not rg["flag"]["actual_one_loop_matrix_beta_system_solved"],"two_loop_not_overclaimed":not rg["flag"]["two_loop_so10_complete"],"detection_not_claimed":not det["flag"]["real_37GHz_detection"]}
    fail=[n for n,o in checks.items() if not o]
    return {"status":"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE" if not fail else "OPEN_GAP_AUDIT_FAILED","n_checks":len(checks),"n_failed":len(fail),"failures":fail,"conditional_cf_region":cf,"fcnc_analysis":f,"yukawa_rg_analysis":rg,"ghz37_package":det,"gap_status":{"exact_unique_full_Ce_Cp_Cn":False,"conditional_aligned_Cf_region":True,"channel_level_FCNC_rates":True,"finite_model_tree_FCNC_absence_proved":False,"pointwise_experimental_FCNC_likelihoods":False,"full_common_scale_Yukawa_RG_fit":False,"two_loop_SO10_threshold_closure":False,"real_37GHz_detection":False},"verdict":"Channel-level mu->e a and K->pi a rates are now explicit. Precision RG, threshold matching, component-specific finite-model FCNC closure, unique C_f, pointwise likelihoods, and experimental detection remain open."}

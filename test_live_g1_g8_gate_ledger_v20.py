...
def test_g1_closed_g2_partial_and_downstream_fail_closed():
    report = ledger.build_report()
    ...
    assert report["gates"]["G2"]["corrections"]["complete_real_field_dimension"] == 486
    assert report["gates"]["G2"]["open_scope"]
...
def test_wave_two_is_the_active_frontier():
    ...
    assert report["closure_waves"][1]["status"] == "ACTIVE_PARTIAL"

...
class OpenGapAuditTests(unittest.TestCase):
    ...
    def test_fcnc_experiments_and_ray_advance_without_model_closure(self):
        r=gaps.fcnc_absence_theorem(); self.assertTrue(r["flag"]["exact_qI_theorem_proved"]); self.assertTrue(r["flag"]["official_NA62_pointwise_limit_ingested"]); self.assertTrue(r["flag"]["generation_dependent_portal_point_excluded_by_NA62"]); self.assertTrue(r["flag"]["conditional_portal_ray_scanned"]); self.assertTrue(r["flag"]["conditional_NA62_survival_boundary_solved"]); self.assertTrue(r["flag"]["form_factor_uncertainty_propagated"]); self.assertTrue(r["flag"]["TWIST_three_published_asymmetry_limits_ingested"]); self.assertTrue(r["flag"]["generation_dependent_portal_point_passes_all_three_TWIST_cases"]); self.assertFalse(r["flag"]["continuous_TWIST_asymmetry_likelihood_implemented"]); self.assertFalse(r["flag"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["flag"]["whole_v20_model_excluded"]); self.assertFalse(r["flag"]["actual_finite_model_fcnc_absence_proved"]); self.assertFalse(r["flag"]["component_specific_uv_chiral_currents_derived"]); self.assertGreater(r["conditional_portal_ray"]["central_survival_boundary"]["y_Q"],1e-6)
    ...
    def test_aggregate_report(self):
        r=gaps.build_report(); self.assertEqual(r["n_failed"],0); self.assertEqual(r["status"],"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE"); self.assertTrue(r["gap_status"]["official_NA62_pointwise_limit"]); self.assertTrue(r["gap_status"]["generation_dependent_kaon_portal_point_excluded"]); self.assertTrue(r["gap_status"]["conditional_portal_ray_scan"]); self.assertTrue(r["gap_status"]["conditional_NA62_yQ_survival_boundary"]); self.assertFalse(r["gap_status"]["full_multidimensional_portal_scan"]); self.assertTrue(r["gap_status"]["TWIST_three_massless_asymmetry_benchmarks"]); self.assertTrue(r["gap_status"]["generation_dependent_muon_portal_point_survives_TWIST_benchmarks"]); self.assertFalse(r["gap_status"]["continuous_TWIST_asymmetry_likelihood"]); self.assertFalse(r["gap_status"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["gap_status"]["finite_model_tree_FCNC_absence_proved"]); self.assertFalse(r["gap_status"]["full_common_scale_Yukawa_RG_fit"]); self.assertFalse(r["gap_status"]["two_loop_SO10_threshold_closure"])

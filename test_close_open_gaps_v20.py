...
class OpenGapAuditTests(unittest.TestCase):
    ...
    def test_fcnc_rates_and_na62_advance_without_model_closure(self):
        r=gaps.fcnc_absence_theorem(); self.assertTrue(r["flag"]["exact_qI_theorem_proved"]); self.assertTrue(r["flag"]["channel_level_amplitudes_implemented"]); self.assertTrue(r["flag"]["official_NA62_pointwise_limit_ingested"]); self.assertTrue(r["flag"]["hierarchical_benchmark_passes_NA62"]); self.assertTrue(r["flag"]["generation_dependent_portal_point_excluded_by_NA62"]); self.assertFalse(r["flag"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["flag"]["whole_v20_model_excluded"]); self.assertFalse(r["flag"]["actual_finite_model_fcnc_absence_proved"]); self.assertFalse(r["flag"]["full_correlated_experimental_likelihood_implemented"]); self.assertFalse(r["flag"]["component_specific_uv_chiral_currents_derived"])
    ...
    def test_aggregate_report(self):
        r=gaps.build_report(); self.assertEqual(r["n_failed"],0); self.assertEqual(r["status"],"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE"); self.assertTrue(r["gap_status"]["channel_level_FCNC_rates"]); self.assertTrue(r["gap_status"]["official_NA62_pointwise_limit"]); self.assertTrue(r["gap_status"]["generation_dependent_portal_point_excluded"]); self.assertFalse(r["gap_status"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["gap_status"]["finite_model_tree_FCNC_absence_proved"]); self.assertFalse(r["gap_status"]["full_correlated_experimental_FCNC_likelihood"]); self.assertFalse(r["gap_status"]["full_common_scale_Yukawa_RG_fit"]); self.assertFalse(r["gap_status"]["two_loop_SO10_threshold_closure"])

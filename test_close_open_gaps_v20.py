...
class OpenGapAuditTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        for module,name in ((push,"PUSH_PHENOMENOLOGY_LIMITS_V20_VERDICT.json"),(common,"COMMON_SCALE_SO10_YUKAWA_V20_VERDICT.json"),(two,"TWO_LOOP_SO10_210_V20_VERDICT.json"),(channel,"CHANNEL_FCNC_RATES_V20_VERDICT.json"),(na62,"NA62_POINTWISE_LIMIT_V20_VERDICT.json"),(twist,"TWIST_MASSLESS_LIMIT_V20_VERDICT.json")):
    ...
    def test_fcnc_experiments_advance_without_model_closure(self):
        r=gaps.fcnc_absence_theorem(); self.assertTrue(r["flag"]["exact_qI_theorem_proved"]); self.assertTrue(r["flag"]["official_NA62_pointwise_limit_ingested"]); self.assertTrue(r["flag"]["generation_dependent_portal_point_excluded_by_NA62"]); self.assertTrue(r["flag"]["TWIST_three_published_asymmetry_limits_ingested"]); self.assertTrue(r["flag"]["hierarchical_benchmark_passes_all_three_TWIST_cases"]); self.assertTrue(r["flag"]["generation_dependent_portal_point_passes_all_three_TWIST_cases"]); self.assertFalse(r["flag"]["continuous_TWIST_asymmetry_likelihood_implemented"]); self.assertFalse(r["flag"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["flag"]["whole_v20_model_excluded"]); self.assertFalse(r["flag"]["actual_finite_model_fcnc_absence_proved"]); self.assertFalse(r["flag"]["component_specific_uv_chiral_currents_derived"])

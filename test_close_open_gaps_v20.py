...
class OpenGapAuditTests(unittest.TestCase):
    ...
    def test_certification_math_and_sector_posterior(self):
        r=gaps.certification_math_package(); self.assertTrue(r["flag"]["mathematical_obstruction_proved"]); self.assertTrue(r["flag"]["conditional_unique_Cf_under_named_axioms"]); self.assertFalse(r["flag"]["unconditional_unique_Cf"]); self.assertTrue(r["flag"]["portal_sector_posterior_derived"]); self.assertFalse(r["flag"]["full_portal_yukawa_posterior_derived"])
    def test_aggregate_report(self):
        r=gaps.build_report(); self.assertEqual(r["n_failed"],0); self.assertEqual(r["status"],"OPEN_GAPS_AUDITED__NO_UNCONDITIONAL_FULL_CLOSURE"); self.assertTrue(r["gap_status"]["official_NA62_pointwise_limit"]); self.assertTrue(r["gap_status"]["conditional_NA62_yQ_survival_boundary"]); self.assertTrue(r["gap_status"]["full_heavy_singular_spectrum_on_ray"]); self.assertFalse(r["gap_status"]["bare_D_is_physical_eigenmass"]); self.assertTrue(r["gap_status"]["lightest_heavy_equals_vS_boundary"]); self.assertFalse(r["gap_status"]["piecewise_component_threshold_matching"]); self.assertTrue(r["gap_status"]["pati_salam_one_loop_yukawa_layer"]); self.assertTrue(r["gap_status"]["portal_sector_posterior_map"]); self.assertFalse(r["gap_status"]["full_multidimensional_portal_scan"]); self.assertFalse(r["gap_status"]["full_portal_yukawa_posterior"]); self.assertTrue(r["gap_status"]["TWIST_three_massless_asymmetry_benchmarks"]); self.assertFalse(r["gap_status"]["continuous_TWIST_asymmetry_likelihood"]); self.assertFalse(r["gap_status"]["all_portal_parameter_space_excluded"]); self.assertFalse(r["gap_status"]["full_common_scale_Yukawa_RG_fit"]); self.assertFalse(r["gap_status"]["two_loop_SO10_threshold_closure"]); self.assertTrue(r["gap_status"]["lab_37GHz_limit_comparison"]); self.assertFalse(r["gap_status"]["real_37GHz_detection"]); self.assertTrue(r["gap_status"]["uniqueness_obstruction_proved"]); self.assertTrue(r["gap_status"]["conditional_unique_Cf_under_named_axioms"]); self.assertFalse(r["gap_status"]["exact_unique_full_Ce_Cp_Cn"])

...
The displayed generation-dependent texture is held fixed and only the positive
Phi-sector Yukawa y_Q is varied.  The quantity |D|=|y_Q|v_Phi/sqrt(2) is
reported explicitly as a bare mass-matrix entry, not as a physical mass
eigenvalue.  Physical heavy singular values are handled independently by
``portal_boundary_heavy_spectrum_v20.py``.
"""
...
def evaluate_point(
    ...
    *,
    ...
    block = portal_block(y_q)
    scenario = rates.scenario_rates(
        f"generation_dependent_yQ_{y_q:.9e}", block, bases
    )
    ...
    return {
        ...
        "bare_D_is_physical_mass_eigenvalue": False,
        "epsilon_QF": [
            float(abs(lam) * portals.VS / (abs(y_q) * portals.VPHI))
            for lam in TEXTURE["lam_Q_F"]
        ],
        "epsilon_QR": float(
            abs(TEXTURE["lam_Q_R"]) * portals.VS / (abs(y_q) * portals.VPHI)
        ),
        "epsilon_SQRbar": float(
            abs(TEXTURE["lam_S_Q_Rbar"])
            * portals.VS
            / (abs(y_q) * portals.VPHI)
        ),
...
def _crossing_intervals(
    ...
    for left, right in zip(rows, rows[1:]):
        a = float(left[key]) - 1.0
        b = float(right[key]) - 1.0
        if a == 0.0:
            out.append((float(left["y_Q"]), float(left["y_Q"])))
        elif a * b < 0.0:
            out.append((float(left["y_Q"]), float(right["y_Q"])))
    if rows and float(rows[-1][key]) == 1.0:
        out.append((float(rows[-1]["y_Q"]), float(rows[-1]["y_Q"])))
    return out
...
def _monotonic_nonincreasing(
    values: list[float], *, relative_slack: float = 1.0e-8
) -> bool:
    return all(
...
def _experimental_inputs() -> tuple[float, float, dict[str, Any]]:
    ...
    return float(limit_result["observed_br_ul_90cl"]), twist_limit, limit_result
...
def scan_ray(
    *,
    ...
    na62_limit, twist_limit, limit_result = _experimental_inputs()
    ...
    monotonic = _monotonic_nonincreasing(
        [float(row["NA62_ratio"]) for row in rows]
    )
    return {
        "configuration": {
            "y_Q_min": y_min,
        ...
        "unique_survival_boundary": (
            crossings[0] if monotonic and len(crossings) == 1 else None
        ),
        "grid_rows": rows,
    }
...
def build_report() -> dict[str, Any]:
    ...
    boundary = boundaries["f0_central"]
    values = [
    ...
    checks = {
        ...
        "central_unique_boundary_found": boundary is not None,
        "central_boundary_rechecks": boundary is not None
        and abs(float(boundary["NA62_ratio"]) - 1.0) < 1.0e-8,
        "high_yQ_endpoint_survives": central["high_endpoint"]["NA62_survives"],
        "f0_band_boundaries_found": all(math.isfinite(value) for value in values),
        "higher_f0_requires_no_smaller_yQ": all(
            right >= left * (1.0 - 1.0e-8)
            for left, right in zip(values, values[1:])
        ),
        "bare_D_not_called_physical_mass": boundary is not None
        and not boundary["bare_D_is_physical_mass_eigenvalue"],
    ...
    return {
        ...
        "flag": {
            ...
            "central_NA62_survival_boundary_solved": boundary is not None,
        ...
        "interpretation": (
            "Along the explicitly fixed texture, varying y_Q produces a unique "
            "monotonic NA62 rate boundary. Larger y_Q, equivalently larger bare "
            "|D| on this ray, suppresses the rate. Bare D is not a physical heavy "
            "mass eigenvalue; the physical singular spectrum is reported separately. "
            "This is not a full portal-space result."
        ),
        "remaining_for_model_level_statement": [
            "compute and use the physical heavy singular spectrum",
...
def write_markdown(report: dict[str, Any]) -> str:
    ...
    if boundary is not None and low is not None and high is not None:
        lines += [
            ...
            f"- Bare `|D|` at boundary: {boundary['bare_D_GeV']:.12e} GeV",
    ...
    lines += ["", "## Scope", "", report["interpretation"], ""]
    return "\n".join(lines)

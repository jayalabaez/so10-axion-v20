...
$\overline{126}_H$ direction, so the complete phase Hessian remains open. At low
...
hash-bound input-manifest checks. A SARAH~4.15.3 run on Wolfram Engine~15.0,
with six SO($N$) defects of that release replaced by exact routines on its
bundled Susyno engine, registers all 19 Lagrangian terms as SO(10), $U(1)_X$
and $\Zs$ invariant with vanishing anomaly sums, and rejects a forbidden
$210_H 10_H^\dagger 10_H$ term and a shifted $X$ charge. This attestation
verifies the model contract (fields, charges, Yukawa and mass terms, singlet
quartics), not the non-singlet quartics of the census below, and it is not
by itself a scalar-vacuum proof. The exact-$X$
...
448-dimensional massive/transverse space. PQ is not gauge-eaten. These ranks
hold at the declared $p+\delta$ point ($p=e_{6789}$, $z_k=e_{2k-2}+ie_{2k-1}$),
whose $\delta=z_1\wedge z_2\wedge z_3\wedge(e_{67}+e_{89})$ is the $Y=-1$ member of
the $(\overline{10},1,3)$, not the SM singlet; at the Pati--Salam G3 witness
they are 34 and 35 (quotients 452 and 451; G4). The neutral
$\Phi_{210}$
...
$H_6$ radial coordinate of that reference point is also not an exact flat: writing its
...
For the $\Phi_{210}+\delta$ Hessian, direct Gaussian-integer and rational
...
$P+\delta$ gauge-orbit matrix has rank 33. After quotienting that kernel, an
...
The final exact global-gap test is decisively negative for
...
this 27-parameter candidate is rejected for G3, which does not
exclude the theory: the fixed-$P$ branch with
$\Sigma\propto\delta$ obeys the exact relation $\Delta V=-m_\perp^2/8$, and its
lower replacement has the wrong stabilizer. A distinct $p:a:\omega=1:1:1$
SU(5)-singlet branch with $\Sigma\propto\delta$ has an explicit global
$\Phi+\Sigma$ SOS and exact rank/nullity $429/33$, but its stabilizer
$SU(3)_c\times SU(2)_L\times U(1)_{T_{3R}}$ is not the Standard Model.  With the chiral vector
$H=(e_6+i e_7)/\sqrt2$, its 28-parameter full-field extension is exactly
stationary and BFB, has symmetry ranks $36/37/38$ and a live positive
448-dimensional quotient with minimum eigenvalue $0.00484459$.  A
source-bound rational-lattice calculation and exact block LDL certificate
prove full Hessian rank/nullity $448/38$, positive semidefiniteness, and a
kernel equal to the 38-dimensional symmetry orbit.  Being non-SM
($\delta$ breaks hypercharge; $|\langle H\rangle|/|\langle\Phi\rangle|=1$), this
point can never close G3.  The literal
single-orbit four-form lemma is false: $F$ and $-F$ obey both projector
equations but are separated by the cubic invariant
$\operatorname{Tr}(A_\Phi^3)=\pm60$ for $\pm F_0$, $|F_0|^2=10$.  Exact mixed
rank $252/252$ excludes the
coupled $-F$ equality branch.  An exact
...
set.  The complete 16-real-dimensional SU(3)-fixed subspace is classified
exactly and contains no further orbit.  At fixed $\Phi=F$ an exact
off-kernel estimate proves the full $\beta=1/20$ gap for arbitrary $H$ and
  ...
  all-zero-residual route is also removed exactly.  On the pure-$\delta$,
  ...
  radial/current value $-7001/995000$, hence the positive margin
\begin{equation}
...
maximally negative pure-$\delta$ sector.  A single exact 4125-projector
covariant, a rational Schur-complement bound, and a three-region radial
completion prove the sharp restricted gap
\begin{equation}
 \Delta V_{\mathrm{pure}\text{-}\delta}\geq \frac{1}{5000},
\end{equation}
with equality at $u=1$, $v=0$ for a signed K\"ahler-square representative.
The earlier four-real-dimensional SU(3) regression is subsumed by the
corrected v21 theorem. At fixed $H=h_-$ and $\Sigma=q/4$, a
source-reconstructed exact positive-Gram identity proves $p(t,\Phi)>0$ away
from the homogeneous origin for every real $\Phi_{210}$. Hence at
$t=1$, $A(\Phi)>3/200$ and the $p$-zero set is empty. This theorem does not
vary $H$ or $\Sigma$ and does not classify the full Hessian or close G3.
At that endpoint an exact tangent
...
nullity 45; its integral nullspace gives an explicit complete basis
of 45 real symmetric quadratic forms commuting with all 15 live actions.
Thus $\dim\!\left(\operatorname{Sym}^2(\Phi_{210})\right)^{\mathrm{SU}(4)}=45$.
The augmented homogeneous representation census is also exact. Its
...
The cubic Schur interface is coordinate-exact. Exact highest-weight
carriers provide all 540 required copies of the ten relevant
$\operatorname{Sym}^2(\Phi_{210})$ irreducible families. After physical
realification they construct all 1414 cubic cross variables in the seven
nonzero real/Hermitian blocks. Their symmetric-Gram multiplication map is an
integer $478\times1414$ matrix of exact rank 478 and kernel dimension 936.
Thus every abstract invariant cubic coefficient vector has a linear preimage.
This is not a PSD result: the reserved zero vector of the
478-coordinate interface is not the physical G3 target and certifies no
vanishing physical cubic right-hand side.
The homogeneous quartic Schur interface is coordinate-exact too.
All 35 complex carrier families, spanning 798 irreducible copies and 22155
carrier dimensions, and all 22 real block pairings give a sparse integer
$6057\times18085$ map with 115641 nonzero entries, exact rank 6057, and
kernel dimension 12028. The legacy v20 assembled physical target is rejected
and kept only as structural provenance; its public entrypoints fail closed.
The corrected standard
positive-Gram map has shape $6585\times19594$, common denominator 256, and
138550 nonzero entries; its ordered-spectral target has denominator
576000 and 512 nonzero entries. All 6585 rational coefficient equalities hold,
and the exact primal has 22 strictly positive real/Hermitian blocks
with 824 positive rational LDL pivots. With the complete invertible
carrier census this proves the arbitrary-real-$\Phi_{210}$ statement at the
fixed $H=h_-$, $\Sigma=q/4$ endpoint only. Global $\Sigma$ and general or full
$H$ remain open for this non-SM point, which can never close G3.
Historical finite-cut and SDP solvers are non-certifying and unused in the
local-minimum proof. G3 is closed only on the Pati--Salam benchmark family:
$(\Phi,\Sigma)\propto(p,z_1\wedge\cdots\wedge z_5)$ is the exact
SM-preserving global vacuum of a distinct, retuned 27-parameter benchmark of the declared
exact-$X$ potential with the light-doublet deformation $\epsilon H^\dagger H$,
$\epsilon>0$ (unique modulo SO(10)$\times$U(1)$_X\times$ accidental
U(1)$_{\rm PQ}$; exact Hessian at $r_0=M_I/M_{\rm GUT}=1/5$,
$|\Phi_{17}|=M_{\rm GUT}$: rank/nullity $451/35$, kernel the symmetry
orbit; six cited classical theorems and six hand-argued steps). The
doublet--triplet and $M_I$ relations are tuned; G4 is open and G6--G8 are
blocked ($\overline{126}$ colour states below $M_I$, no electroweak breaking,
RG-anchor content, benchmark quartic $127/64$, Yukawas); no internal
candidate is approved. This work neither validates nor excludes the
complete model.
...
flavour fits, RG evolution of the Wilson tensors, radiatively stable
$v_\Phi/v_S$, doublet--triplet and $M_I$ hierarchies, a thermal-restoration
calculation, an ultimate

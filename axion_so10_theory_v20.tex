...
15 skew actions on $\Phi_{210}$ are source-bound.  Exact intertwiners further
decompose the complexified 210 into 25 carriers of total rank 210 and prove
that $\dim\!\left(\operatorname{Sym}^2(\Phi_{210})\right)^{\mathrm{SU}(4)}=45$.
These statements certify only one representation-theory input for the full
augmented SU(4)-equivariant degree-2 Schur/SOS SDP.  The 45-dimensional
quadratic invariant sector is not the complete Gram problem: every real or
Hermitian isotypic block and all homogenizing cross terms remain to be
constructed.  No arbitrary-$\Phi$ bound, G3 closure, or whole-model exclusion
follows from this census.

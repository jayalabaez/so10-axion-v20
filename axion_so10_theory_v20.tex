...
The construction is an anomaly-free field theory to the reduced Planck
cutoff, not a string completion; flavour/Clebsch fits, hierarchy
stabilization and the string network remain open. Three fail-closed
...
is dominantly $\arg S$, phase-locked by
$\overline{126}_H^2 10_H^2 S^2$. We gauge $\Zs := \mathrm{U(1)}_{\rm PQ}
\bmod 17$. Spectators: $k$ identical $16_s\oplus\overline{16}_s$ pairs
...
so the axion's QCD anomaly and low-energy couplings are unchanged.
...
field-theory completion to the Planck cutoff rather than an ultimate
quantum-gravity construction.
...
The continuous embedding passes its stated tests: all local anomalies
...
and adds a v20 40-check engine with a controlled nonzero-exit path and

...
$210_H 10_H^\dagger 10_H$ term and a shifted $X$ charge. This hash-bound
attestation makes the model contract consistent; the global scalar-potential
gate G3 remains open. This also does not mean
...
exact gap--curvature relation $\Delta V=-m_\perp^2/8$, and its lower replacement
...
the kernel is exactly the 38-dimensional symmetry orbit.  This point is not the
physical vacuum: $H$ lies in an electroweak doublet with
$|\langle H\rangle|/|\langle\Phi\rangle|=1$, and $\Sigma$ and $S$ sit at
$M_{\rm GUT}/5$ rather than at $M_I$.  Retuning only the $10_H$ mass
term leaves one massless doublet with no tree-level coupling to heavy modes, so
$\lambda_{\rm eff}=1$ and one-loop running gives $m_h\simeq173\GeV$; any
nonnegative matching quartic gives $m_h\gtrsim134\GeV$, so the physical G3
target needs a negative GUT-scale threshold.  The declared portal
$\lambda_{HS}|H|^2|S|^2$, zero in the benchmark, supplies it at tree level,
$\lambda_{\rm eff}=\lambda_H-\lambda_{HS}^2/(4\lambda_S)$, and
$\lambda_{HS}\simeq2.03$ reproduces the one-loop Standard Model value.  The literal
...
$H$, the full Hessian, G3 closure, and whole-model conclusions remain open. The

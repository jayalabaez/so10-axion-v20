...
quartics), not the non-singlet quartics of the census below; G3 remains
open. This also does not mean
...
the $(\overline{10},1,3)$, not the SM singlet; they match SM counts but must be
recomputed at an SM vacuum. The neutral
$\Phi_{210}$
...
false, but this does not exclude the theory: the fixed-$P$ branch with
...
the kernel is exactly the 38-dimensional symmetry orbit.  Being non-SM
($\delta$ breaks hypercharge; $|\langle H\rangle|/|\langle\Phi\rangle|=1$), this
point cannot pass the final G3 test.  The literal
...
off-kernel estimate proves the full $\beta=1/20$ gap for arbitrary $H$ and
  ...
  all-zero-residual route can also be removed exactly.  On the pure-$\delta$,
...
subsumed by the corrected v21 theorem. At fixed $H=h_-$ and $\Sigma=q/4$, a
source-reconstructed exact positive-Gram identity proves $p(t,\Phi)>0$ away
from the homogeneous origin for every real $\Phi_{210}$. Consequently, at
...
The cubic Schur interface is now coordinate-exact. Exact highest-weight
...
This statement is not a PSD result: the reserved zero vector attached to the
478-coordinate interface is not the physical G3 target and does not certify a
vanishing physical cubic right-hand side.
The homogeneous quartic Schur interface is now coordinate-exact as well.
...
and retained only as structural provenance; its public report, render, write,
and command-line entrypoints now fail closed. The corrected standard
positive-Gram map has shape $6585\times19594$, common denominator 256, and
138550 nonzero entries; its corrected ordered-spectral target has denominator
576000 and 512 nonzero entries. All 6585 rational coefficient equalities hold,
and the exact primal consists of 22 strictly positive real/Hermitian blocks
with 824 positive rational LDL pivots. Together with the complete invertible
carrier census, this proves the arbitrary-real-$\Phi_{210}$ statement at the
fixed $H=h_-$, $\Sigma=q/4$ endpoint only. Global $\Sigma$, general or full
$H$, G3 closure, and whole-model conclusions remain open. The
historical finite-cut and SDP solvers remain non-certifying and are not used in
the local-minimum proof. Separately,
$(\Phi,\Sigma)\propto(p,z_1\wedge\cdots\wedge z_5)$ is an exact SOS global SM
minimum of a retuned benchmark potential (unique modulo
SO(10)$\times$U(1)$_X\times$U(1)$_{\rm PQ}$; $M_I$ and
doublet--triplet splitting tuned; $\overline{126}$ colour states below $M_I$;
field content does not reproduce the RG anchor fixing $M_I$; benchmark quartic
$127/64$, i.e.\ $m_h\approx195$~GeV; not yet in the G3 gate). Therefore this work
neither validates nor excludes the complete model.

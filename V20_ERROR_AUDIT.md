...
The package checks pass, but several checks only reproduce assumptions already
embedded in the engine. This audit imports **no** v20 engine code.
...
| Incomplete Lagrangian | Extra gauge- and PQ-invariant operators exist, including $PR\,10_H$, $\overline{Q}R S^\dagger$, and $SQ\overline R$. They modify spectra, projected currents and Yukawa alignment. |
| “Full amplitude” | $6.04\times10^{-47}$ is a **unit-coefficient kernel** for one selected graph—not a physical prediction. |
| Fermion moving-basis identity | $Q_{\rm proj}+iB^\dagger dB/d\alpha=\mathbf1$ is basis dependent. The physical projected current remains $Q_{\rm proj}=\mathbf1-4W$ and can be flavour off-diagonal. |
| Flavour extraction | The earlier fit used `eigh` on a non-Hermitian complex-symmetric Majorana matrix and omitted $U_e^\dagger$. Correct Takagi/PMNS extraction removes the claimed viable $v_R=v_S$ result. |
...
- Reproducibility of the **aligned-current** ERT-like benchmark only.
...
2. **Needs external inputs** — complete portal tensors and their SM-Yukawa
   alignment, a global corrected Higgs/flavour fit, precision threshold/RG
   and correlated hadronic matching,
   two-loop thresholds, Wilson coefficients, $v_\Phi/v_S$ stabilization, and
   string-network cosmology.
